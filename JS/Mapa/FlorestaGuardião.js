...
const portalDois = new DirectLink("portalAzul", "dimensaoEsquecida.html", [1, 3], 2);
const desPortalDois = new Description(portalDois.getElemento(), "???");

const posPilares = [[2, 2], [2, 4], [5, 3]]
for(let i =0; i<3; i++){
    let novoPilar = new FixedEntity([posPilares[i][0], posPilares[i][1]], 2);
    setSprite(novoPilar.getElemento(), "entidade", "pilar-floresta");
}

const posTochas = [[1, 2], [4, 2], [1, 4], [4, 4]]
for(let i =0; i<4; i++){
